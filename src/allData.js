...
useEffect(() => {
    ...
    const handleClick = (index)=>{
        ...
        axios.delete(`https://bank-server-rc04.onrender.com/delete/${deleteItem._id}`).then(() => {
...
function handleEdit(item) {
    ...
    async function handleUpdate() {
        try {
        await axios.put(`https://bank-server-rc04.onrender.com/update/${editId}`, formData);
...
console.log(products)
    ...
    {products.map((item , index)=><tr>
        ...
        <button style={{border:"none" , fontSize:'30px' }}><FaUserEdit style={{marginRight:'.0003%', color:'#ba551'}} title='Edit User' onClick={() => handleEdit(item)}/> <FaWindowClose style={{color:"red"}} onClick={ ()=>{handleClick(index) }} title='Delete User'/></button>
    </tr>
    )
    }
